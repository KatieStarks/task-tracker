...
The `home-routes` handler in the `task-tracker.routes.home` namespace
...
The `home-page` function will in turn call the `task-tracker.layout/render` function
...
The rest of this page is rendered by ClojureScript found in the `src/cljs/task_tracker/core.cljs` file.



#### Organizing the routes

The routes are aggregated and wrapped with middleware in the `task-tracker.handler` namespace:
...
Request middleware functions are located under the `task-tracker.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `task-tracker.dev-middleware` namespace found in
